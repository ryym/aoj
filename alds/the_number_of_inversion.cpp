// マージソートを使う。
// 例えばある配列をそれぞれ整列済みのL,Rに分割してマージする時、
// R[0]が先頭に来るようなら、Lの全要素はR[0]よりも大きい事になるので、
// その場合にはLの要素数だけ反点数となるペアがある事になる。
// これを一般化すると、マージ処理において、R[i]をマージ配列に
// 置く度に、その時点でLに残っている要素数を合計していけば、
// そのマージにおける反点数の数がわかる。
//  L[j], R[i]
//  L[j + 1], R[i]
//  ..
